# 비즈니스 커뮤니케이션 개요

## 1. 비즈니스 커뮤니케이션 개념 및 요소

---

### 가) 비즈니스 커뮤니케이션 정의

- 어떤 일을 일정한 목적과 계획을 가지고 짜임새 있게 지속적으로 경영함
- 통상적으로 일반 기업이 하는 제반 활동
- 사업 아이테이나 사업 분야를 말하는 경우도 있음

### 나) 비즈니스 커뮤니케이션 요소

![image](6-project/business_communication_process.png)

- 핵심 요소는 공유하고자 하는 정보, 송신자와 수신자, 매체이다.
- 정보를 담기 위한 인코딩, 정보를 획득하기 위한 디코딩
- 잡음(noise)가 발생할 수 있음
- 피드백 필수적

### 다) 비즈니스 커뮤니케이션 유형

![image](6-project/business_communication_type.png)

- 문서
  - 문자를 사용
  - 단방향 의사전달
- 언어
  - 말을 사용
  - 실시간 피드백 가능
- 비언어
  - 표정, 목소리 등 사용
  - 이성보다는 감성에 호소
- 가장 중요한 것은 몸짓(비언어)
- 보이지 않는 부분이 매우 중요하게 의사전달에 영향을 미침

## 2. 비즈니스 커뮤니케이션 방법

- 수평적
  - 동료 사이
- 수직적
  - 상사와 부하직원 간
  - 상향식
  - 하향식

### 소통채널과 정보충실도

![image](6-project/communication_channel.png)

# 비즈니스 문제해결 기법

### 비즈니스 문제해결 프로세스

1. 문제 정의

- 문제의 인식
- 문제정의 기술

2. 원인 분석

- 프로파일링 및 분석기법 활용

3. 대인 개발 및 평가

- 평가기준 & 가중치 기반 평가

4. 해결안적용 및 피드백

- 실행 및 평가
- 피드백 및 개선

### 창의적 사고기법

1. 브레인스토밍
2. 여섯가지 생각모자

   - 하얀 모자
     - 중립적으로 사실이나 데이터 설명 역할
   - 빨간 모자
     - 감정적인 발상 역할
   - 검은 모자
     - 부정적이거나 반대하는 역할
   - 노란 모자
     - 긍정적인 역할
   - 초록 모자
     - 창조적으로 새로운 아이디어를 내는 역할
   - 파란 모자
     - 냉정하고 객관적으로 결과 정리 역할

3. 랜덤 워드

- 독특한 아이디어가 나올때까지 단어를 제시하고, 그 중 무작위적으로 조합하여 과제와 연결

4. 스캠퍼(SCAMPER)

- Substitute, Combine, Adapt, Modify or Magnify, Put to other side, Eliminate, Rearrange or Reverse
- 위 행위들을 해보며 아이디어 발상

5. TRIZ(Theory Of Inventive Problem Solving)

- 특정 문제를 추상화를 통해 일반화한 후, 일반적 해결 모형을 찾아 해결방안 도출

### 논리적 사고기법

1. MECE(Mutually Exclusive and Collectively Exhausive)

   - 상호 중복 없이 전체적으로 빠진 것이 없는
   - 어떤 정보나 메시지를 중복이나 누락 없이 구성하는 것
   - 전체의 합이 모든 경우의 수를 포함하도록 처리하는 것

2. 로직트리

   - MECE 사고방식에 따라 주요항목을 트리 형태로 분해한 것

3. 80:20의 법칙

   - 핵심문제 20%를 해결하면 전체 문제 80%가 해결된다.

### 합리적 의사결정 기법

1. 다요소의사결정

   - 다양한 요소 고려

2. AHP(분석적 계층화기법)

- 상황을 트리구조로 분할하고 이를 단계적으로 분석
- 분할된 문제를 각각의 쌍대비교를 통해 평가하는 방식

# 비즈니스 문서 작성기법

---

## 비즈니스 문서의 개념과 종류

---

### 비즈니스 문서의 종류

1. 비즈니스 문서의 개념

- 프로세스 기획문서, 업무보고서, 회의록, 결과 및 현황보고서 등 비즈니스 활동에 필요한 모든 형태의 문서

2. 비즈니스 문서의 종류

- 보고서
  - 정보보고서
  - 관리보고서
- 의견서
- 기획서
- 계획서
- 설명서

## 비즈니스 문서의 작성원칙

- 최종소비자의 관점에서 쉬운 이해
- 간결.명료하게 작성
- 명확한 근거 반영
- 논리적인 기술

## 비즈니스 문서작성 시 체크리스트

![image](6-project/business_document_checklist.png)

# 테크니컬 문서 작성방법

## 테크니컬 문서의 특징

1. 명확성
2. 정확성
3. 체계성

## 테크니컬 문서의 종류

1. 프로젝트 관리 기술 문서

- 사업계획서
- 정보제공요청서
- 제안 요청서
- 제안서
- 프로젝트 관리 계획서
- Requirement Traceability Matrix
- 프로젝트 완료 보고서

2. 프로젝트 개발 기술 문서

- 분석서
- 설계서
- 개발 문서
- 테스트 문서
- 설명서

## 테크니컬 문서 작성의 원칙과 방법

### 명확한 테크니컬 문서 작성

- 연관성
- 일관성
- 정확성
- 적합성

## 테크니컬 문서 작성

### 사업계획서

1. 일반현황
2. 목표
3. 내용
4. 사업 방법
5. 재무
6. 기대효과

### 정보제공요청서

1. 사업 개요
2. 발주업체 정보
3. 주요 요구사항

### 제안요청서

1. 사업의 개요
2. 현황 및 문제점
3. 사업추진 방향
4. 제안 요청 내용

### 제안서

1. 제안 개요
2. 제안업체 일반
3. 기술부문
4. 사업관리부문
5. 지원부문
6. 기타

### Requirement Traceability Matrix

1. 사용자 요구사항
2. 분석 산출물
3. 설계 산출물
4. 구현 및 테스트 산출물

### 분석서

1. 일반 사항
2. 일반 기술사항
3. 상세 내용

### 설계서

1. 일반 사항
2. 구조 설계
3. 프로그램 설계
4. 인터페이스설계
5. 자료 설계

### 테스트 설계서

1. 테스트 항목
2. 테스트 케이스 및 데이터

### 설명서

1. 일반 사항
2. 사용법 및 설치/점검 방법
3. 유지보수 및 문제해결
4. 기타

### 프로젝트완료 보고서

1. 프로젝트결과요약
2. 프로젝트 교훈
3. 프로젝트 통계

# 프레젠테이션

---

## 프레젠테이션 구상 및 준비절차

### 프레젠테이션 목적

- 정보의 전달
- 고객 설득
- 행동 유발

### 프레젠테이션 과정

1. 목표/주제설정
2. 3P분석
3. 내용구성/설계
4. 자료수집/분석
5. 전달매체로 작성
6. 리허설
7. 발표
8. 사후처리(질의응답, 피드백)

### 시나리오 작성

1. 시나리오 내용 구성
2. 시나리오 분량 구성

## 프레젠테이션 자료 작성 기법

### 전체 자료 구성

- 오프닝 -> 목차 -> 본문 -> 마무리
- 본문 구성방식
  - 요청 순서 형(요청한 대로)
  - 핵심 이슈형(핵심 이슈 중심으로)
  - 발표 평가표 형(평가표 내용 중심으로)

### 발표 시나리오 구상

1. 목적, 대상, 장소에 적합한 논리전개
2. 목차구성
3. 논리흐름의 점검
4. 결론 정의

# 프로젝트 이해

---

## 프로젝트 개념

### 프로젝트 특징

- 일시적 노력
- 고유한 제품과 서비스 제공
- 점진적 상세화
- 프로젝트 범위, 원가 및 시간 제약
- 자원과 품질 제공

## 프로젝트 조직구조와 역할

### 프로젝트 조직구조

- 기능 조직
- 프로젝트 중심 조직
- 매트릭스 조직(기능 조직 + 프로젝트 중심 조직)

### 프로젝트 전담 조직(PMO)

1. PMO 타입
   - 내부조직 구성(기업 내부 프로젝트 관리자)
   - 외부조직 구성(외부 프로젝트 관리자)
   - 혼합 유형
2. PMO 책임과 역할
   - 기상대 모델
     - 소수의 전문 인력
     - 진행상황에 대한 정보만 제공
     - 프로젝트 관리지식, 방법론 개발 및 전파
   - 코치형 모델
     - 기상대 모델의 확장 개념
     - 방법론과 소프트웨어 도구 사용 전파
     - 커뮤니티 운영
     - 프로젝트 성과 기록 및 모니터링
   - 컨트롤 타워 모델
     - 모든 프로젝트에 관여
     - 의사결정에 깊이 개입
     - 모든 프로젝트는 동일한 방법론 적용
     - 중앙집중적 관리
3. PMO Position
   - 하나의 프로젝트만 관리하는 경우 전담 유형
   - 복수의 프로젝트를 관리하는 경우 일시 유형

# 프로젝트 프로세스와 관리

## 프로젝트 생명주기

1. 착수
2. 계획
3. 실행
4. 감시 및 통제
5. 종료

- 증분형과 진화형 생명주기
  ![image](6-project/project_lifecycle.png)
