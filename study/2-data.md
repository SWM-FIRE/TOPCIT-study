...
> 데이터베이스 운영 도중 장애가 발생한 경우 장애 발생 이전으로 일관성과 무결성을 복원

## 데이터베이스 장애와 복구의 개념

### 데이터베이스 장애의 유형

- 트랜잭션 장애 : 내부적 / Deadlock 의 오류로 트랜잭션을 완료할 수 없음
- 시스템 장애 : 전원 / 하드웨어 / 소프트웨어의 고장
- 디스크 장애 : 디스크 스토리지의 일부 또는 전체가 붕괴
- 사용자 장애 : 사용자들의 이해 부족으로 발생

### 데이터베이스 복구의 기본원리

- Archive / Dump : 다른 저장장치에 복사하여 저장
- Log / Journal : 변경될 때마다 변경되는 데이터의 과거값과 갱신값을 따로 저장

### 데이터베이스 복구조치 유형

- REDO : 아카이브 사본 + 로그 -> 완료
- UNDO : Log + Backward 취소연산 -> 시작

## 데이터베이스 장애 복구 방법

### 데이터베이스 장애 복구 방법

- 로그 기반 기법 : Redo, Undo를 활용한 로그파일을 이용한 복구 (느림)
- 검사점 복구 기법 : Undo를 사용한 로그파일과 검사점을 이용한 복구 (로그보다 빠름)
- 그림자 페이징 기법 : 그림자 테이블을 교체하는 방법 (빠름)

# 데이터베이스 분석 이해

## 데이터웨어하우스

> 내부 데이터와 외부 데이터를 주제별로 통합하여 별도의 프로그래밍 없이 즉시 분석할 수 있도록 하는 통합 시스템 혹은 데이터베이스

### 특징

> OLTP 시스템을 위한 OLTP의 데이터 저장소와는 다르다

- 주제지향적 : 특정 주제의 데이터만을 저장하여 그 외의 데이터는 포함하지 않음
- 통합적 : 표준화를 통하여 데이터의 정합성과 물리적 통일성을 갖는 구조
- 시계열적 : 과거와 현재의 경향에 대한 분석 및 미래에 대한 예측을 위하여 Snapshot 형태로 보유
- 비휘발성 : 한번 적재된 후 삭제 및 수정이 불가능한 조회 전용 데이터베이스

## 데이터웨어하우스 모델링

> 다양한 관점에서 빠른 성능으로 분석할 수 있도록 데이터 분석 관점의 데이터 모델링 기법

- 팩트 테이블 : 관련성 높은 Measure들의 집합
- 차원 테이블 : 부속 테이블로서 팩트를 분석하는 관점으로, 다양한 관점의 데이터 분석이 가능

## ETL

> 데이터를 소스 시스템에서 추출하여 정제 및 변환 후 데이터웨어하우스에 적재하는 전 고자ㅓㅇ

- Extraction(추출) : 원본 파일 혹은 데이터베이스로부터 데이터웨어하우스에 저장될 데이터를 추출
- Transformation(변환) : 추출한 데이터를 정제하고 데이터 형태에 맞게 변환
- Loading(적재) :
...
> 기존 데이터베이스 관리도구의 데이터 수집, 저장, 분석을 넘어서는 다양한 종류의 대규모 데이터로부터 저렴한 비용으로 가치를 추출, 수집, 발굴, 분석을 하는 기술

### 3V

- Volume(크기) : SW의 처리 한계를 넘어서는 페타바이트이상의 단위 데이터를 의미
- Velocity(속도) : 수집, 처리, 저장, 분석을 실시간에 가까운 속도로 처리
- Variety(다양성) : 정형, 반정형, 비정형 데이터등 다양한 데이터를 구분

### Life Cycle

1. 수집 : 모든 디바이스 및 시스템에서 나오는 데이터를 수집할 수 있는 기술
   - DBMS
   - 센서
   - FTP
   - HTTP
2. 저장/처리 : 수집된 대용량 데이터를 저장하여 분산처리 시스템에 처리할 수 있는 기술
   - DFS
   - NoSQL
   - 분산 병렬처리
3. 분석 : 기업에 도움이 될 수 있고, 생활에 도움을 줄 수 있는 분석방법
   - 로지스틱 회귀분석
   - 의사결정트리 분석
   - 신경망 분석
   - 텍스트 마이닝
...
> 비정형, 초고용량 데이터 처리를 위해 쓰기속도에 중점을 둔 다수 서버에 데이터 복제 및 분산 저장 등 수평적 확장이 가능한 비-관계형 분산 데이터 저장소
...
- Key/Value
- Column Family : 칼럼 패밀리에 행으로 데이터를 저장
- Document : Key-Value 데이터베이스의 Value부분에 저장
- Graph : 엔트리 속성을 노드, 관계를 엣지로 표현
...
> 셋 중 최대 두개의 특징밖에 가질 수 없다

# 인공지능 이해

> 학습능력과 추론능력, 지각능력, 자연언어의 이해능력 등을 프로그램으로 실현한 기술

> 튜링테스트 : 얼마나 인간과 비슷하게 대화할 수 있는지 판별하는 테스트

## 기계학습

- 지도학습 : 원하는 결과가 표현된 학습 데이터를 이용한 학습 방법
  - 신경망
  - HMM
  - 의사결정트리
  - MLP
- 비지도 학습: 원하는 결과가 표현되지 않는 학습 데이터를 이용한 학습 방법
  - Clustering
  - K-Means
  - HC
- 강화학습 : 잘한 행동에 대하여 칭찬을 받고, 잘못된 행동에 대하여 벌을 받는 학습 방법
